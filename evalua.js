...
function parsearTrades(raw) {
  ...
  for (let r = 0; r < raw.length; r++) {
    ...
    // Buscar columna símbolo (español o inglés)
    ...
    if (sIdx >= 0) {
      ...
      colBen  = row.findIndex(h => h === 'beneficio' || h === 'profit' || h === 'benefit');
      colAp   = row.findIndex(h => h === 'fecha/hora' || h === 'open time' || h === 'time');
      // Precio cierre: segundo "precio" o "close price"
      ...
      // Fecha cierre: segunda "fecha/hora"
  ...
  // Si no encontramos cabeceras, intentar modo posicional MT5 estándar
  if (headerRow === -1) {
    // Buscar primera fila que tenga XAUUSD o GOLD
    for (let r = 0; r < raw.length; r++) {
      const sym = String(raw[r][2] || '').toUpperCase();
      if (sym.includes('XAU') || sym.includes('GOLD')) {
        headerRow = r - 1;
        colAp = 0; colSym = 2; colTipo = 3; colVol = 4; colPe = 5; colCi = 8; colPc = 9; colBen = 12;
        break;
      }
    }
  }
  ...
  function toDate(v) {
    ...
    // Formato MT5: 2026.04.13 19:35:07
  ...
  for (let i = headerRow + 1; i < raw.length; i++) {
    ...
    // Ignorar filas que no son trades reales (sin buy/sell)
    const tipo = String(row[colTipo] || '').toLowerCase().trim();
    if (tipo && tipo !== 'buy' && tipo !== 'sell' && tipo !== 'compra' && tipo !== 'venta') continue;

    const ben = toNum(row[colBen]);
    const vol = toNum(row[colVol]);
    const pe  = toNum(row[colPe]);
    const pc  = toNum(row[colPc]);

    if (isNaN(ben) || isNaN(vol) || vol === 0) continue;

    const apDt = toDate(row[colAp]);
    const ciDt = toDate(row[colCi]);

    let hora = 0, dia = 1, durMin = 60;
    if (apDt && !isNaN(apDt)) {
      hora = apDt.getHours();
      dia  = apDt.getDay();
      if (ciDt && !isNaN(ciDt)) durMin = (ciDt - apDt) / 60000;
    }

    const fp = String(row[1] || '') + '_' + String(row[colAp] || '') + '_' + pe + '_' + vol;
    trades.push({ ben, vol, pe, pc: isNaN(pc) ? pe : pc, puntos: Math.abs((isNaN(pc)?pe:pc) - pe), ganadora: ben > 0, hora, dia, durMin, fp });
  }
  return trades;
}
