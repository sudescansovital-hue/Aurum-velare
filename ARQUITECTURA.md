...
- Fix EA (`EA_Aurum_Tracker_FIX.mq5`):
  ...
  - `OnTimer` procesa la cola completa **una vez al día** (`HoraEnvioDiario`, input, por defecto 23h servidor) — no en tiempo real, ya que el objetivo es registro/auditoría, no visualización en vivo en Aurum.
  ...
  - Persistencia en disco: la cola de eventos pendientes se escribe a `MQL5\Files\Common\aurum_cola_<cuenta>.txt` en cada cambio, y se recupera al arrancar — sobrevive a cierres de MT5/PC apagado (importante para swing trades de varios días).
...
- Estado de pruebas: EA probado en vivo en cuenta demo Roderas (152034) — apertura, parcial, SL a break even, todo registrado y sin duplicados tras el fix. Indicador de sesiones corregido pero **aún no probado en el chart** (pendiente).
...
|---|---|---|
| sudescansovital-hue/Aurum-velare | main | `66a87b6` fix: puntos ignora SL=0 o SL≈entrada, usa precio de cierre como fallback |
